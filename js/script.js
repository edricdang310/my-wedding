    ...
    'use strict'; window.FPConfig = { delay: 0, ignoreKeywords: [], maxRPS: 3, hoverDelay: 50 }; $(window).on("load", function () { $('.spinner').fadeOut(); $('.preloader-area').delay(350).fadeOut('slow'); }); jQuery(document).ready(function ($) {
        ...
        function applyBgImage() {
            ...
            sliderBgSetting.each(function (indx) {
                ...
                if (isMobile && $(this).attr("data-background-mobile")) {
                    bg = $(this).attr("data-background-mobile");
                }
                if (bg) {
        ...
        function renderMessages(messages) {
            ...
            if (messages && messages.length > 0) {
                messages.forEach(function (msg, idx) {
                    var words = msg.message.trim().split(/\s+/);
                    var displayMsg = msg.message;
                    var moreHtml = '';
                    
                    if (words.length > 30) {
                        var shortText = words.slice(0, 30).join(' ') + '...';
                        var readMoreBtn = '<a class="read-more-btn btn-more-' + idx + '" onclick="document.querySelector(\'.short-msg-' + idx + '\').style.display=\'none\'; document.querySelector(\'.full-msg-' + idx + '\').style.display=\'inline\'; this.style.display=\'none\'; return false;">Xem thêm</a>';
                        var readLessBtn = '<a class="read-more-btn" style="margin-left:5px;" onclick="document.querySelector(\'.short-msg-' + idx + '\').style.display=\'inline\'; document.querySelector(\'.full-msg-' + idx + '\').style.display=\'none\'; document.querySelector(\'.btn-more-' + idx + '\').style.display=\'inline-block\'; return false;">Ẩn bớt</a>';
                        
                        displayMsg = '<span class="short-msg-' + idx + '">' + shortText + '</span>' + 
                                     '<span class="full-msg-' + idx + '" style="display:none;">' + msg.message + readLessBtn + '</span>';
                        moreHtml = readMoreBtn;
                    }

                    html += '<div class="message-item">';
                    html += '<h4>' + msg.name + '</h4>';
                    html += '<p>' + displayMsg + moreHtml + '</p>';
                    html += '</div>';
                });
            } else {
                html = '<p style="text-align:center;color:#999;padding:20px;">Chưa có lời chúc nào. Hãy là người đầu tiên!</p>';
            }
        ...
        if ($('#messagesList').length) {
            ...
            $.ajax({
                ...
                error: function () {
                    console.log('Bạn cần chạy `node server.js` để tải dữ liệu.');
                }
            });
        }
        ...
        $('#guestbookForm').on('submit', function (e) {
            ...
            if (name && message) {
                ...
                $.ajax({
                    ...
                    error: function () {
                        alert('Lỗi! Bạn cần khởi chạy máy chủ bằng lệnh `node server.js` thì mới gửi được.');
                    },
                    complete: function () {
