...
if (biicore.bgMusic) {
	...
	setTimeout(function () {
		if (document.getElementsByClassName("bii-player")[0]) {
	...
	// Tự động phát nhạc khi chạm, click hoặc cuộn
	function autoPlayInteraction(event) {
		...
		if (audioPlayer.paused) {
			audioPlayer.play().then(function () {
				...
				console.log("Autoplay failed via " + event.type + ":", error);
			});
		} else {
			cleanupListeners();
		}
	...
	function cleanupListeners() {
		window.removeEventListener('click', autoPlayInteraction);
		window.removeEventListener('touchstart', autoPlayInteraction);
		window.removeEventListener('scroll', autoPlayInteraction);
		window.removeEventListener('wheel', autoPlayInteraction);
		window.removeEventListener('touchmove', autoPlayInteraction);
	}

	window.addEventListener('click', autoPlayInteraction);
	window.addEventListener('touchstart', autoPlayInteraction);
	window.addEventListener('scroll', autoPlayInteraction);
	window.addEventListener('wheel', autoPlayInteraction);
	window.addEventListener('touchmove', autoPlayInteraction);

	// Thử tự động phát nhạc sau 3 giây
	setTimeout(function () {
		autoPlayInteraction({ type: 'timeout' });
	}, 3000);
	document.write(`
	<style type="text/css">
